...
def phrase_prob(phrase_combinations,ngram_freq1,ngram_freq2,corpus):
	prob_scores = []
	for phraseX in phrase_combinations:
		...
		for x in range(0,len(words)-1):
			if ((words[x] in ngram_freq1) and (words[x+1] in ngram_freq1[words[x]])) and (words[x] in corpus):
				prob_score_fwd=prob_score_fwd*((ngram_freq1[words[x]][words[x+1]]+0.5)/(corpus[words[x]]+len(corpus)))
			elif words[x] in corpus:
				prob_score_fwd=prob_score_fwd*((0.5)/(corpus[words[x]]+len(corpus)))
			elif ((words[x] in ngram_freq1) and (words[x+1] in ngram_freq1[words[x]])):
				prob_score_fwd=prob_score_fwd*((ngram_freq1[words[x]][words[x+1]]+0.5)/(len(corpus)))
			else:
				prob_score_fwd=prob_score_fwd*((0.5)/(len(corpus)))
		prob_scores.append(s_score*(prob_score_fwd)/math.exp(edit_dist+1))
	return prob_scores
		...
		for cword in candidate_words:
			if cword not in parts and cword.title() not in parts:
				...
				if new_phrase != ' '.join(parts):
					phrase_combinations.append((new_phrase,compute_edit_distance(cword,parts[index]),soundex_score2(s1,s2)))
	prob_scores = phrase_prob(phrase_combinations,ngram_freq1,ngram_freq2,corpus)
	ls = [(x[0],y) for y,x in sorted(zip(prob_scores,phrase_combinations), reverse=True)]
	return ls[:3]
...
if __name__ == "__main__":
	...
	for phrase in infile.readlines():
		#print phrase
		#phrase = ''.join(ch for ch in phrase if ch.isalnum())
		if phrase[-1]=='\n':
			phrase = phrase[:-1]
		if phrase[-1]=='.':
			phrase = phrase[:-1]
		...
		if len(wordX)>0:
			...
			if len(req_dict)>0:
				...
				for correction in sorted_x:
					if correction[0].lower() not in dictionary:
						continue
					else:
